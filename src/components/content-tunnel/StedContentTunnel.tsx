...
export function StedContentTunnel({ items = sourceCardFixtures, className = '' }: { items?: StedContentItem[]; className?: string }) {
  ...
  return <section ref={root} className={`sct ${className}`} style={{ '--slots': count } as CSSProperties} aria-label="Your saved internet, together in Sted" data-paused={!inView} onPointerMove={move} onPointerLeave={reset}>
    <div className="sct-window">
      <motion.div className="sct-camera" aria-hidden="true" style={{ x: reducedMotion ? 0 : x, y: reducedMotion ? 0 : y }}>
